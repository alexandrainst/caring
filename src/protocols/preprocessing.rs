...
pub fn dealer_prepross<F:PrimeField>(mut rng: rand::rngs::mock::StepRng, known_to_each: Vec<usize>, number_of_triplets: usize, number_of_parties: usize) -> (Vec<SpdzContext<F>>, SecretValues<F>){
    ...
    for i in 0..number_of_parties{
        ...
        let p_context = SpdzContext{
            opened_values: vec![],
            closed_values: vec![],
            params: spdz::SpdzParams{mac_key_share, who_am_i: i},
    ...
    for _ in 0..number_of_triplets{
        ...
        for i in 0..number_of_parties-1{
            ...
            let ai_share = spdz::Share{val:ai, mac:a_mac_i};
            let bi_share = spdz::Share{val:bi, mac:b_mac_i};
            let ci_share = spdz::Share{val:ci, mac:c_mac_i};
