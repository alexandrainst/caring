    ...
    G: std::ops::Mul<F, Output = G>,
{
    ...
    let shares : Vec<VerifiableShare<F>> = ids.iter().map(|i| {
        VerifiableShare(p1.eval(i), p2.eval(i))
    }).collect();

    let commitments : Box<[G]> = p1.0.iter().zip(p2.0.iter()).map(|(&a,&b)| *g*a + *h*b).collect();
...
pub fn verify<F, G>(
    ...
    commit: &Commitment<G>,
    PedersenGenParams(g,h): &PedersenGenParams<G>
) -> bool where F: Field, G: Group + std::ops::Mul<F, Output=G> {
...
pub fn reconstruct<F: Field>(shares: &[VerifiableShare<F>], ids: &[F]) -> F {
    ...
    let shares : Vec<_> = shares.iter().zip(ids).map(|(share, id)| super::shamir::Share{x: *id, y: share.0})
        .collect();
...
mod tests {
    ...
    use rand::{thread_rng, rngs::mock::StepRng, CryptoRng};
    ...
    fn simple() {
        ...
        let gens =PedersenGenParams(RistrettoPoint::generator(), RistrettoPoint::random(&mut rng));
